...
namespace Project1.NewFolder1
{
    ...
    public class CalculatePenalty: ICalculatePenalty
    {
        ...
        public double CalculatePenalty(Countries Countries, PageInput input)
        {
            int workDays = GetBusinessDays(Countries, input); //Getting businessdays and storing in workdays
            double penalty;
            if (workDays > 10)
            {
                penalty = (workDays - 10) * (50 * Countries.CountryExchange); //calculating penalty using exchange rate for respective coutries
            }
            else
            {
                penalty = 0;
        ...
        {
            double penalty = CalculatePenalty(input.Countries, input);
            PageOutput output = new PageOutput(penalty, input.Countries.Currency);// Showing Penalties WRT countries currency
            return output;
        }
